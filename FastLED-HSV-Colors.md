 ...
 * The bottom grayscale bar is the Radiance of each color: the total amount of light emitted.  The FastLED color maps have very uniform radiance across the color map, and a correspondingly (mostly) uniform power consumption across colors.
 * The top grayscale bar is the Luminance of each color: Luminance is Radiance weighted by the sensitivity of the human eye to each component of the light, and represents the apparent brightness of each color, more or less.
